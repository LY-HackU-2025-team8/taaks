...
public interface TaskReminderRepository extends JpaRepository<TaskReminder, Long> {
     ...
     * @return A list of TaskReminder entities that fall within the specified time range.
     *         Returns an empty list if no task reminders are found in that range.
     */
    List<TaskReminder> findByScheduledAtGreaterThanEqualAndScheduledAtBefore(ZonedDateTime scheduledAtStart, ZonedDateTime scheduledAtEnd);
