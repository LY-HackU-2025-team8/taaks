...
create table task_remind (task_id integer not null, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP not null, id bigint generated by default as identity, scheduled_at TIMESTAMP WITH TIME ZONE not null, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP not null, primary key (id));
create index idx_notification_target_token_user_id on notification_target_token (user_id);
create index idx_task_remind_task_id on task_remind (task_id);
create index idx_task_remind_scheduled_at on task_remind (scheduled_at);
...
alter table if exists task_remind add constraint FKon4oug8d4cy2r70tss04x2yck foreign key (task_id) references taak_task;
